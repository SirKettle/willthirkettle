...
Swiper = function(selector, params, callback) {
	...
	var m_bTouchControlsEnabled = true;
	...
	_this.enableTouchControls = function(bEnable){
		
		
		m_bTouchControlsEnabled = bEnable;
		
		Log.Log("enableTouchControls:" + m_bTouchControlsEnabled)
	};
	...
	_this.startAutoPlay = function() {
		...
		if (params.onPlayStateIsPlaying) params.onPlayStateIsPlaying(true);
	}
	_this.stopAutoPlay = function() {
		...
		if (params.onPlayStateIsPlaying) params.onPlayStateIsPlaying(false);
	}
	if (params.autoPlay) {
		_this.startAutoPlay();
	}
	...
	function onTouchStart(event) {
		...
		if (m_bTouchControlsEnabled === false){
			
			Log.Log("onTouchStart m_bTouchControlsEnabled - " + m_bTouchControlsEnabled);
		...
		if (!_this.support.touch || event.targetTouches.length == 1 ) {
			...
			//CallBack
			if (params.onTouchStart) params.onTouchStart(_this, thisEvent)
	...
	function onTouchMove(event) {
		...
		if (m_bTouchControlsEnabled === false){
			...
			Log.Log("onTouchMove m_bTouchControlsEnabled - " + m_bTouchControlsEnabled);
		...
		if (!_this.support.touch || event.touches.length == 1) {
			...
			//Callbacks
			if (params.onTouchMove) params.onTouchMove(_this, thisEvent)
	...
	function onTouchEnd(event) {
		...
		if (params.onTouchEnd) params.onTouchEnd(_this, thisEvent);
		_this.callPlugins('onTouchEnd');
	}
